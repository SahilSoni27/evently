...
class EmailService {
  ...
  private async configureTransporter() {
    try {
   ...
   */
  async sendEmailWithTemplate(templateName: string, context: Record<string, any>, to: string, subject: string): Promise<boolean> {
    try {
  ...
  async sendEmail(emailData: EmailTemplateData): Promise<boolean> {
    ...
    try {
      ...
      const text = emailData.text || htmlToText(emailData.html, {
        wordwrap: 130,
        selectors: [
  ...
  async sendTestEmail(to: string): Promise<boolean> {
    const testEmailData: EmailTemplateData = {
      to,
  ...
  async sendWaitlistConfirmation(data: {
    ...
    return this.sendEmailWithTemplate(
      ...
      data,
      data.to,
      `You're on the waitlist: ${data.eventName}`
    );
  }
  ...
  async sendWaitlistPromotion(data: {
    ...
    return this.sendEmailWithTemplate(
      ...
      data,
      data.to,
      `🎉 Ticket Available: ${data.eventName}`
    );
  }

  // Batch email sending with rate limiting
  async sendBatchEmails(emails: EmailTemplateData[], batchSize: number = 10, delayMs: number = 1000): Promise<void> {

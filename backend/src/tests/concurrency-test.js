/**
 * Simple Concurrency Test Script for Event Booking System (JavaScript version)
...
const TEST_CONCURRENCY_LEVELS = [5, 10, 15, 25];
...
class ConcurrencyTester {
  ...
  async createTestEvent() {
    try {
      ...
      console.log(
        `✅ Test event created: ${this.testEvent.name} (ID: ${this.testEvent.id})`
      );
  ...
  async simulateConcurrentBookings(concurrency) {
    if (!this.testEvent) {
    ...
    console.log(
      `   Expected successful bookings: ${Math.min(
        concurrency * TICKETS_PER_USER,
        this.testEvent.capacity
      )}`
    );
    ...
    const bookingPromises = selectedUsers.map((user) =>
      this.attemptBooking(user)
    );
  ...
  async attemptBooking(user) {
    const startTime = Date.now();

    try {
      // Generate unique idempotency key for this booking attempt
      const idempotencyKey = `test_${user.id}_${
        this.testEvent.id
  ...
  async verifyEventCapacity() {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/events/${this.testEvent.id}`
      );
  ...
  async getBookingCount() {
    try {
      ...
      const bookings = response.data.data.bookings.filter(
        (booking) =>
          booking.eventId === this.testEvent.id &&
  ...
  analyzeResults(results, concurrency) {
    ...
    console.log("\n📊 Results Analysis:");
    console.log(
      `   Successful bookings: ${successful.length}/${concurrency} (${(
        (successful.length / concurrency) *
        100
      ).toFixed(1)}%)`
    );
    console.log(
      `   Failed bookings: ${failed.length}/${concurrency} (${(
        (failed.length / concurrency) *
        100
      ).toFixed(1)}%)`
    );
    ...
    if (failed.length > 0) {
      console.log("\n❌ Failure Reasons:");
      const failureReasons = failed.reduce((acc, result) => {
    ...
    return {
      ...
      maxResponseTime,
      minResponseTime,
      failureReasons: failed.reduce((acc, result) => {
        const reason = result.error || "Unknown error";
        acc[reason] = (acc[reason] || 0) + 1;
        return acc;
      }, {}),
  ...
  async resetEventCapacity() {
    try {
      // Reset event capacity via admin endpoint
      await axios.delete(
        `${API_BASE_URL}/admin/events/${this.testEvent.id}/reset-bookings`,
        {
          headers: { Authorization: `Bearer ${this.adminToken}` },
        }
      );
      ...
      console.warn(
        "⚠️  Could not reset event capacity automatically:",
        error.response?.data?.message || error.message
      );
    }
  }
  ...
  async runFullTest() {
    try {
      ...
      for (const concurrency of TEST_CONCURRENCY_LEVELS) {
        ...
        const isValid =
          totalBooked + currentCapacity === EVENT_CAPACITY &&
          totalBooked <= EVENT_CAPACITY;
        console.log(`   ✅ No overselling: ${isValid ? "PASSED" : "FAILED"}`);

        if (!isValid) {
          console.error(
            `❌ OVERSELLING DETECTED! Booked: ${totalBooked}, Remaining: ${currentCapacity}, Total should be: ${EVENT_CAPACITY}`
          );
        }
  ...
  printSummary(results) {
    ...
    results.forEach((result) => {
      ...
      console.log(
        `   Success rate: ${(result.analysis.successRate * 100).toFixed(1)}%`
      );
      console.log(
        `   Avg response time: ${result.analysis.avgResponseTime.toFixed(2)}ms`
      );
    ...
    console.log(
      `🏆 OVERALL RESULT: ${
        hasOverselling || !allSuccessful ? "❌ FAILED" : "✅ PASSED"
      }`
    );
    console.log("=".repeat(60));
    ...
    if (!hasOverselling && allSuccessful) {
      console.log(
        "✅ All tests passed! No overselling detected and good success rates."
      );
    }
  }
}

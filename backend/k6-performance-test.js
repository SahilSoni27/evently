...
export const options = {
  scenarios: {
    ...
    // Stress test - Peak traffic (Moderate load)
    stress_test: {
      ...
      stages: [
        ...
        { duration: "1m", target: 15 }, // Maintain spike
...
export default function (data) {
  ...
  const phase =
    __ENV.PHASE ||
    (__VU % 3 === 0 ? "booking" : __VU % 3 === 1 ? "browsing" : "waitlist");
  ...
  sleep(Math.random() * 2 + 1); // Random sleep 1-3 seconds (prevent rate limiting)
}
...
function testBookingFlow(baseUrl, eventId, user) {
  ...
  const isBookingSuccess =
    bookingRes.status === 200 || bookingRes.status === 201;
...
function testWaitlistFlow(baseUrl, eventId, user) {
  ...
  check(waitlistRes, {
    "Waitlist request processed": (r) =>
      r.status === 200 || r.status === 201 || r.status === 400,
    "No server errors on waitlist": (r) => r.status < 500,
  });

  // Check waitlist status
  const waitlistStatusRes = http.get(
    `${baseUrl}/api/events/${eventId}/waitlist/my-position`,
    {
      headers: { Authorization: `Bearer ${token}` },

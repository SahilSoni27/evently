  ...
  scenarios: {
    event_management_load: {
      ...
      stages: [
        ...
        { duration: '1m', target: 50 },   // Peak organizer activity
        { duration: '2m', target: 50 },   // Peak sustained
...
function eventManagementFlow(baseUrl, token) {
  ...
  if (Math.random() > 0.7) { // 30% chance to create event
    const eventData = generateEventData();
    const createRes = http.post(`${baseUrl}/api/events`, JSON.stringify(eventData), {
      headers,
    ...
    if (creationSuccess) {
      ...
      const updateRes = http.put(`${baseUrl}/api/events/${eventId}`, JSON.stringify(updateData), {
        headers,
  ...
  if (Math.random() > 0.6) { // 40% chance
    const searchRes = http.get(`${baseUrl}/api/events?search=test&category=conference`, {
      headers: { 'Authorization': `Bearer ${token}` },
...
function generateEventData() {
  const eventTypes = ['Conference', 'Concert', 'Workshop', 'Seminar', 'Festival'];
  const categories = ['technology', 'music', 'business', 'education', 'entertainment'];

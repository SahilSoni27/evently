...
class SearchService {
    ...
    async searchEvents(filters) {
        const { query = '', category, venue, minPrice, maxPrice, startDate, endDate, tags = [], availableOnly = false, sortBy = 'startTime', sortOrder = 'asc', limit = 20, offset = 0 } = filters;
        // Build where clause
        const where = {};
    ...
    buildTextSearchCondition(query) {
        ...
        const conditions = terms.map(term => `(name ILIKE '%${term}%' OR description ILIKE '%${term}%' OR venue ILIKE '%${term}%')`);

...
class SeatGenerationService {
    ...
    static async generateSeatsForEvent(data) {
        ...
        try {
            ...
            if (!venueId) {
                const newVenue = await prisma_1.default.venue.create({
                    data: {
                        name: venueName,
                        capacity,
            ...
            for (const sectionData of sections) {
                const section = await prisma_1.default.venueSection.create({
                    data: {
                        venueId: venueId,
                ...
                const seats = this.generateSeatsForSection(currentSeatNumber, sectionData.capacity, seatsPerRow, sectionData.seatType);
                // Create seats in database
                const createdSeats = await prisma_1.default.seat.createMany({
                    data: seats.map(seat => ({
     ...
     */
    static generateDefaultSections(capacity) {
        if (capacity <= 50) {
     ...
     */
    static generateSeatsForSection(startNumber, sectionCapacity, seatsPerRow, seatType) {
        const seats = [];
     ...
     */
    static async getEventSeatMap(eventId) {
        const event = await prisma_1.default.event.findUnique({
            where: { id: eventId },
            include: {
                venueDetails: {
        ...
        return {
            ...
            availableSeats: seatMap.reduce((total, section) => total + section.seats.filter(seat => !seat.isBooked && !seat.isBlocked).length, 0),
            bookedSeats: seatMap.reduce((total, section) => total + section.seats.filter(seat => seat.isBooked).length, 0)
        };
    }
}
exports.SeatGenerationService = SeatGenerationService;

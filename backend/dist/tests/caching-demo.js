...
async function demonstrateCaching() {
    ...
    let analytics = await redis_1.default.get('analytics:demo');
    if (!analytics) {
        console.log('❌ Cache miss - fetching from database...');
        analytics = await fetchExpensiveAnalytics();
        await redis_1.default.set('analytics:demo', analytics, 300); // 5 min TTL
    ...
    const cachedAnalytics = await redis_1.default.get('analytics:demo');
    if (cachedAnalytics) {
    ...
    await redis_1.default.set('user:1', userData, 600);
    await redis_1.default.set('event:1', eventData, 600);
    const [cachedUser, cachedEvent] = await Promise.all([
        redis_1.default.get('user:1'),
        redis_1.default.get('event:1')
    ]);
    console.log('👤 Cached user:', cachedUser);
    console.log('🎪 Cached event:', cachedEvent);
    // Test 5: Cache invalidation
    console.log('\n5️⃣ CACHE INVALIDATION');
    console.log('Before deletion - exists:', await redis_1.default.exists('analytics:demo'));
    await redis_1.default.del('analytics:demo');
    console.log('After deletion - exists:', await redis_1.default.exists('analytics:demo'));
    console.log('\n==============================');

...
exports.ConcurrencyTester = void 0;
...
class ConcurrencyTester {
    ...
    async createTestEvent() {
        try {
            ...
            const response = await axios_1.default.post(`${API_BASE_URL}/events`, eventData, {
    ...
    async createTestUsers(count) {
        ...
        const userPromises = Array.from({ length: count }, async (_, i) => {
            ...
            try {
                // Register user
                const registerResponse = await axios_1.default.post(`${API_BASE_URL}/auth/register`, userData);
                // Login to get token
                const loginResponse = await axios_1.default.post(`${API_BASE_URL}/auth/login`, {
    ...
    async attemptBooking(user) {
        ...
        try {
            ...
            const response = await axios_1.default.post(`${API_BASE_URL}/bookings`, bookingData, {
    ...
    async verifyEventCapacity() {
        try {
            const response = await axios_1.default.get(`${API_BASE_URL}/events/${this.testEvent.id}`);
    ...
    async getBookingCount() {
        try {
            const response = await axios_1.default.get(`${API_BASE_URL}/bookings?limit=100`, {
                headers: { Authorization: `Bearer ${this.adminToken}` }
            });
            const bookings = response.data.data.bookings.filter((booking) => booking.eventId === this.testEvent.id && booking.status === 'CONFIRMED');
    ...
    async resetEventCapacity() {
        try {
            ...
            await axios_1.default.delete(`${API_BASE_URL}/admin/events/${this.testEvent.id}/reset-bookings`, {
                headers: { Authorization: `Bearer ${this.adminToken}` }
            });
            // Reset event capacity
            await axios_1.default.put(`${API_BASE_URL}/events/${this.testEvent.id}`, {
...
exports.ConcurrencyTester = ConcurrencyTester;
// Run the test
async function main() {

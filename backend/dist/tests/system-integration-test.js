...
async function testCompleteSystem() {
    ...
    try {
        await redis_1.default.set('system:test', { status: 'connected', time: Date.now() });
        const result = await redis_1.default.get('system:test');
    ...
    try {
        await prisma_1.default.$queryRaw `SELECT 1 as test`;
    ...
    try {
        const emailQueue = new bullmq_1.Queue('email-notifications');
        const analyticsQueue = new bullmq_1.Queue('analytics-processing');
        const reminderQueue = new bullmq_1.Queue('event-reminders');
        // Add test jobs
    ...
    try {
        ...
        // Test cache set/get
        await redis_1.default.set('analytics:overview', analyticsData, 300); // 5 min TTL
        const cachedData = await redis_1.default.get('analytics:overview');
        console.log('✅ Analytics Cache: Working');
        console.log('   Cached data:', cachedData);
        // Test cache invalidation
        await redis_1.default.del('analytics:overview');
        const afterDelete = await redis_1.default.get('analytics:overview');
    ...
    try {
        ...
        const result = await prisma_1.default.$queryRaw `SELECT COUNT(*) as count FROM "Waitlist"`;
        ...
        await redis_1.default.set(waitlistCacheKey, mockWaitlistData, 600); // 10 min TTL
        const cachedWaitlist = await redis_1.default.get(waitlistCacheKey);
    ...
    try {
        ...
        const promises = [];
        for (let i = 0; i < 10; i++) {
            promises.push(redis_1.default.set(`perf:test:${i}`, { data: `test-${i}` }));
        }
        await Promise.all(promises);
        const getPromises = [];
        for (let i = 0; i < 10; i++) {
            getPromises.push(redis_1.default.get(`perf:test:${i}`));
        }
        ...
        // Cleanup
        for (let i = 0; i < 10; i++) {
            await redis_1.default.del(`perf:test:${i}`);
        }
    }
    catch (error) {

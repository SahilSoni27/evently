...
// Concurrent booking scenario (realistic load)
export const options = {
  scenarios: {
    concurrent_booking_rush: {
      ...
      startVUs: 1,
      stages: [
        { duration: "30s", target: 10 }, // Gentle ramp up
        { duration: "1m", target: 25 }, // Peak concurrent users
  ...
  thresholds: {
    concurrent_booking_success: ["rate>0.3"], // At least 30% should succeed
    http_req_duration: ["p(95)<5000"], // 95% under 5 seconds
    seatSelectionTime: ["p(90)<3000"], // Seat selection under 3s
...
function concurrentBookingRush(baseUrl, eventId, user) {
  ...
  sleep(Math.random() * 0.5 + 0.2); // 0.2-0.7s random delay
  ...
  if (loginRes.status === 429) {
    sleep(2); // Wait 2 seconds for rate limit reset
    return;
  }
  ...
  if (!actualSuccess && bookingRes.status === 400) {
    const waitlistRes = http.post(
      `${baseUrl}/api/events/${eventId}/waitlist`,
      JSON.stringify({}),
      {
        headers: {
          "Content-Type": "application/json",
    ...
    const waitlistSuccess =
      waitlistRes.status === 200 || waitlistRes.status === 201;
  ...
  // Brief pause to simulate user reading response + prevent rate limiting
